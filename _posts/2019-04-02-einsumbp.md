...
$$
\Im[I\circ dP] = \Im[I\circ(dC+dD)] ~~~~ ~~~~~~~~~~~~~~~~~~~~~~~~~\text{Eq. 1}
$$
This **the missing diagonal imaginary part** is definitely not trivial, but has been ignored for a long time until [@refraction-ray](https://github.com/tensorflow/tensorflow/issues/13641#issuecomment-526976200) (Shixin Zhang) mentioned as solved it. Let's first focus on the off-diagonal contributions from $dU$
$$
...
$$
Should be independent of the choice of gauge $\Lambda$, which is defined as ${\rm diag}(e^\phi, ...)$
$$
\begin{align}
...
$$
\begin{align}
\bar A_{U+V}^{(\rm imag)} &= U^*\Lambda_J S^{-1}V^T
\end{align}
$$
**Thanks  [@refraction-ray](https://github.com/tensorflow/tensorflow/issues/13641#issuecomment-526976200) (Shixin Zhang) for sharing his idea in the first time.  His arXiv preprint is coming out soon.**
...
&+ \frac 1 2 U (I\circ(U^\dagger\overline U)-h.c.)S^{-1}V^\dagger
\end{align}
$$
