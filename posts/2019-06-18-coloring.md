...
~~~
<img src="/assets/images/cpetersen.png" width="500px" alt="petersen-graph"/>
~~~
...
~~~
<img src="/assets/images/petersenijk.png" width="500px" alt="petersen-graph-labeled"/>
~~~
...
所以当`einsum`不再省略输出指标，我们可以定义更加丰富的运算，比如
...
这些新引入的规范拓展了**张量网络**，这类记号一般认为与机器学习概率图中的**factor graph** 等价，也在某些文献中被称为**拓展-张量网络**[引文 1]，Andreas Peter也有个不错的[Blog](https://nextjournal.com/under-Peter/julia-summer-of-einsum)是关于这类记号的约定和实现方案。
...
这里我们用了短短两行代码实现了上面的15重循环。上面用到了爱因斯坦求和记号的库叫做[`OMEinsum`](https://github.com/under-Peter/OMEinsum.jl)，是19年的Google Summer of Code项目，安装需要在Julia的Pkg模式（在REPL输入`]`进入）输入`pkg> add OMEinsum#master`。
...
从这些例子，大家是否观察到一些特点呢？爱因斯坦求和的反向传播过程也是一个爱因斯坦求和，只是输入和输出以及它们对应的指标交换而已，证明就不放到这里，有兴趣的读者可以参考这个[blog](https://giggleliu.github.io/2019/04/02/einsumbp.html)。
